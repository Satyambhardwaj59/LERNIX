...
const AddCourse = () => {
  ...
  const [lectureDeteails, setLectureDetails] = useState({
    lectureTitle: '',
    lectureDuration: '',
    lectureUrl: '',
    isPreviewFree: false,
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex items-center flex-wrap justify-between'>
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input type="number" placeholder='0' onChange={e => setCoursePrice(e.target.value)} value={coursePrice} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required />
          </div>
        ...
        <div>
          <p>Discount %</p>
          <input type="number" placeholder='0' min={0} max={100} onChange={e => setDiscount(e.target.value)} value={discount} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' />
        </div>
          ...
          {showPopup && (
            <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                ...
                <div className='mb-2'>
                  <p>Lecture Title</p>
                  <input type="text" className='mt-1 block w-full border rounded py-1 px-2' value={lectureDeteails.lectureTitle} onChange={(e) => setLectureDetails({ ...lectureDeteails, lectureTitle: e.target.value })} />
                </div>

                <div className='mb-2'>
                  <p>Duration (minutes)</p>
                  <input type="number" className='mt-1 block w-full border rounded py-1 px-2' value={lectureDeteails.lectureDuration} onChange={(e) => setLectureDetails({ ...lectureDeteails, lectureDuration: e.target.value })} />
                </div>

                <div className='mb-2'>
                  <p>Lecture URL</p>
                  <input type="text" className='mt-1 block w-full border rounded py-1 px-2' value={lectureDeteails.lectureUrl} onChange={(e) => setLectureDetails({ ...lectureDeteails, lectureUrl: e.target.value })} />
                </div>

                <div className='mb-2'>
                  <p>is Preview Free?</p>
                  <input type="checkbox" className='mt-1 scale-125'
                  checked={lectureDeteails.isPreviewFree}  onChange={(e) => setLectureDetails({ ...lectureDeteails, isPreviewFree: e.target.value })} />
                </div>
